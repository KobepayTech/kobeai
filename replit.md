...
- **Subscription payments via M-Pesa STK push**: parent app `/subscription` page lets a parent pay their child's monthly plan. Flow: parent app → local school server `POST /v1/parent/subscriptions/pay` → central `POST /central/v1/payments/initiate` → row in `subscription_payments` (status=pending, checkout_request_id) → demo simulator auto-completes after `DEMO_STK_DELAY_MS` (4s) with ~92% success rate → on success the same DB transaction renews `student_subscriptions` (+30 days, status=active, last_payment_at=now). The row claim is idempotent (`UPDATE ... WHERE status='pending' RETURNING`) so concurrent timer/webhook callbacks settle exactly once. Bursar dashboard polls `GET /v1/bursar/subscription-payments` (admin/teacher auth, proxies to central) every 5s for a live collection feed with TSh totals + receipts. Parent payment-status reads (`GET /v1/parent/subscriptions/payment/:id`) proxy to central and filter by the parent's owned `student_code`s to block IDOR. Production swap-in: replace the `setTimeout(completePayment, ...)` with a real `/mpesa/callback` webhook handler (Daraja / Selcom / Azampay) that calls the same idempotent `completePayment` function.
